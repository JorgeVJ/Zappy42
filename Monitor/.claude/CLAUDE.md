...
| `/animals` | `.claude/commands/animals.md` | Cualquier cambio en el sistema de fauna decorativa: colocación (`AnimalSystem`), movimiento/paseo por capas (dominio navegable `IAnimalDomain`/`AquaticDomain`, locomoción `AnimalLocomotion`, comportamiento `IAnimalBehavior`/`WanderBehavior`, base `Animal`), animación procedural de huesos (`Fish`), añadir especies/medios (terrestre/aéreo) o el futuro Utility AI, y portar/eliminar el sistema |
